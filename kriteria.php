    ...
    <div class="admin-main">
      <nav class="navbar admin-navbar navbar-expand bg-white">
        <div class="container-fluid px-3 px-lg-4">
          <button class="sidebar-toggle" type="button" data-sidebar-toggle aria-controls="" aria-expanded="true"
          ...
          <div class="page-heading mb-4">
            <div class="page-heading-copy">
              ...
              <div>
                ...
                <p class="text-muted mb-0">Atur bobot prioritas kepentingan serta tipe preferensi kriteria penilaian
                  smartphone.</p>
              </div>
            </div>
          </div>
          ...
          <div class="row g-4">
            <div class="col-12 col-lg-4">
              <div class="card border-0 shadow-sm p-4 bg-white rounded-3 h-100">
                <h3 class="h5 mb-2 fw-bold text-dark"><i class="bi bi-plus-circle me-2 text-primary"></i>Tambah Kriteria
                </h3>
                <p class="text-muted small mb-4">Tambahkan indikator penilaian baru beserta parameter perhitungan
                  pembobotan.</p>

                <form method="POST" action="kriteria.php">
                  <div class="mb-3">
                    <label class="form-label small fw-semibold text-secondary">Kode Kriteria</label>
                    <input type="text" name="id_kriteria" class="form-control bg-light" placeholder="Contoh: C1"
                      required>
                  </div>
                  <div class="mb-3">
                    <label class="form-label small fw-semibold text-secondary">Nama Parameter</label>
                    <input type="text" name="nama_kriteria" class="form-control bg-light"
                      placeholder="Contoh: Kapasitas RAM" required>
                  </div>
                  <div class="mb-3">
                    <label class="form-label small fw-semibold text-secondary">Nilai Bobot (W)</label>
                    <input type="number" step="0.01" name="bobot" class="form-control bg-light"
                      placeholder="Contoh: 0.25 atau 5" required>
                  </div>
                  <div class="mb-4">
                  ...
                  <button type="submit" name="add_kriteria" class="btn btn-primary w-100 py-2 fw-semibold shadow-sm"><i
                      class="bi bi-save me-2"></i>Simpan Kriteria</button>
                </form>
              </div>
            </div>
                      ...
                      if (mysqli_num_rows($qk) > 0) {
                        while ($rk = mysqli_fetch_assoc($qk)) {
                          ...
                          <tr>
                            <td class="ps-4">
                              <span
                                class="badge bg-primary-subtle text-primary fw-bold px-2.5 py-1.5 border border-primary-subtle">
                            ...
                            <td>
                              <?php if ($rk['tipe'] == 'benefit'): ?>
                                <span class="badge bg-success-subtle text-success border border-success-subtle px-2 py-1"><i
                                    class="bi bi-graph-up-arrow me-1"></i>Benefit</span>
                              <?php else: ?>
                                <span class="badge bg-danger-subtle text-danger border border-danger-subtle px-2 py-1"><i
                                    class="bi bi-graph-down-arrow me-1"></i>Cost</span>
                              <?php endif; ?>
                            </td>
                            <td class="text-end pe-4">
                              <button type="button"
                                class="btn btn-outline-primary btn-sm border-0 shadow-none px-2 py-1 me-1"
                                data-bs-toggle="modal" data-bs-target="#editModal<?php echo $rk['id_kriteria']; ?>">
                                <i class="bi bi-pencil-square"></i>
                              </button>

                              <a href="kriteria.php?del_kriteria=<?php echo $rk['id_kriteria']; ?>"
                                class="btn btn-outline-danger btn-sm border-0 shadow-none px-2 py-1"
                                onclick="return confirm('Hapus parameter kriteria <?php echo $rk['nama_kriteria']; ?>?')">
                                <i class="bi bi-trash3-fill"></i>
                              </a>
                            </td>
                          </tr>

                          <div class="modal fade" id="editModal<?php echo $rk['id_kriteria']; ?>" tabindex="-1"
                            aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered">
                              <div class="modal-content border-0 shadow rounded-3">
                                <div class="modal-header bg-light border-bottom">
                                  <h5 class="modal-title fw-bold text-dark"><i
                                      class="bi bi-pencil-square me-2 text-primary"></i>Edit Kriteria
                                    <?php echo $rk['id_kriteria']; ?></h5>
                                  <button type="button" class="btn-close shadow-none" data-bs-shadow="none"
                                ...
                                <form method="POST" action="kriteria.php">
                                  <div class="modal-body p-4">
                                    ...
                                    <div class="mb-3">
                                      <label class="form-label small fw-semibold text-secondary">Nama Parameter</label>
                                      <input type="text" name="nama_kriteria" class="form-control bg-light"
                                        value="<?php echo $rk['nama_kriteria']; ?>" required>
                                    </div>
                                    <div class="mb-3">
                                      <label class="form-label small fw-semibold text-secondary">Nilai Bobot (W)</label>
                                      <input type="number" step="0.01" name="bobot" class="form-control bg-light"
                                        value="<?php echo $rk['bobot']; ?>" required>
                                    </div>
                                    <div class="mb-2">
                                      <label class="form-label small fw-semibold text-secondary">Jenis Tipe</label>
                                      <select name="tipe" class="form-select bg-light" required>
                                        <option value="benefit" <?php echo ($rk['tipe'] == 'benefit') ? 'selected' : ''; ?>>
                                          Benefit (Semakin besar semakin baik)</option>
                                        <option value="cost" <?php echo ($rk['tipe'] == 'cost') ? 'selected' : ''; ?>>Cost
                                          (Semakin kecil semakin baik)</option>
                                      </select>
                                    </div>
                                  </div>
                                  <div class="modal-footer bg-light border-top p-3">
                                    <button type="button" class="btn btn-secondary border-0 btn-sm px-3"
                                      data-bs-dismiss="modal">Batal</button>
                                    <button type="submit" name="edit_kriteria"
                                      class="btn btn-primary btn-sm px-3 fw-semibold"><i
                                        class="bi bi-check-circle me-1"></i>Simpan Perubahan</button>
                                  </div>
                                </form>
                              </div>
                            </div>
